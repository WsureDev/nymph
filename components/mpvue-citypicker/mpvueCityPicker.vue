		...
		<div class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
			...
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange">
				<block>
					...
					<picker-view-column v-show="deep>1">
						<div class="picker-item" v-for="(item,index) in cityDataList" :key="index">{{item.label}}</div>
					</picker-view-column>
					<picker-view-column v-show="deep>2">
		...
		methods: {
			...
			_$emit(emitName) {
				let pickObj = {
					...
					labelArray: this._getLabelArray(),
			...
			_getLabel() {
				let pcikerLabel = this.provinceDataList[this.pickerValue[0]].label;
				if(this.deep>1)
				{
					pcikerLabel += ( '-' + this.cityDataList[this.pickerValue[1]].label );
				} 
				if(this.deep>2) {
					pcikerLabel += ( '-' + this.areaDataList[this.pickerValue[2]].label );
				}
				return pcikerLabel;
			},
			_getCityCode() {
